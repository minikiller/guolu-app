...
class TbApi:
    ...
    def get_customer(self, name):
        ...
        """
        customers = self.get(
            f"/api/customers?limit=99999&textSearch={name}", f"Can't find customer with name '{name}'")
    ...
    def get_device_token(self, device):
        ...
        json = self.get(f"/api/device/{device_id}/credentials",
                        f"Error retreiving device_key for device '{device_id}'")
    ...
    def get_telemetry(self, device, telemetry_keys, startTime=None, endTime=None, interval=None, limit=None, agg=None):
        ...
        if endTime is None:
            # Unix timestamp, now, convert to milliseconds
        ...
        params = "/api/plugins/telemetry/DEVICE/" + device_id + "/values/timeseries?keys=" + keys + "&startTs=" + \
            str(int(startTime)) + "&endTs=" + str(int(endTime)) + "&interval=" + \
            str(interval) + "&limit=" + str(limit) + "&agg=" + agg
        error_message = "Error retrieving telemetry for device '" + device_id + "' with date range '" + \
            str(int(startTime)) + "-" + str(int(endTime)) + \
            "' and keys '" + keys + "'"
    ...
    def validate_response(response, msg):
        try:
            ...
            # Append our response to the exception to make it easier to figure out WTF went wrong
            ex.args += (f"RESPONSE BODY: {response.content.decode('utf8')}",)
            raise
